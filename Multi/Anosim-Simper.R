...
NMDS1<- metaMDS(Distancias1, distance = "jaccard")
plot(NMDS1)  
NMDS1$stress
...
ggplot(DF2,aes(x=MDS1,y=MDS2, col = Usos)) + ggtitle("Riqueza vs Variables bióticas y ambientales") + geom_point(cex=6) +  
        ...
        #geom_text(data = Vectores, aes(x = NMDS1, y = NMDS2+0.03), label = row.names(Vectores), colour = "navy", fontface = "bold",position=position_jitter(width=0.02,height=0.012)) + #Descomentar esta linea para los nombres de los vectores
        #geom_text(data=DF2,aes(x=MDS1,y=MDS2,label=ID),alpha=0.5, nudge_y = -0.020) + #Descomentar esta linea para los nombres de los puntos
        annotate(geom="text", x=0.55, y=-0.50, label="Stress =",color="red",size=6) +
        annotate(geom="text", x=0.55, y=-0.56, label=round(NMDS1$stress,digits = 4),color="red",size=8)
